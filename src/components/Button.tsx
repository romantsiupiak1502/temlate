...
const ButtonBase = styled.button<IButtonTextStyledProps>`
  ...
  background-color: ${ ({ theme, isDisabled, isSecondary }) =>
          isDisabled
                  ? theme.colors.backgroundDarkGray
                  : isSecondary
                  ? theme.colors.white
  ...
  border: ${ ({ theme, isSecondary }) => isSecondary ? `1px solid ${ theme.colors.gray }` : '0' };
  outline: 0;
...
export const ButtonLong: React.FC<IButtonProps> = props => {
  const { text, isDisabled, isSecondary, onClick, className } = props;
  return (
    <ButtonBase isSecondary={ isSecondary } isDisabled={ isDisabled } onClick={ onClick } isLong={ true }
                className={ className }>
      <ButtonTextStyled isSecondary={ isSecondary } isDisabled={ isDisabled }>{ text }</ButtonTextStyled>
    </ButtonBase>
  )
